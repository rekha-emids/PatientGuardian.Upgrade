...
import ManageConnection from "./index";
...
jest.mock('react-navigation-redux-helpers', () => ({reactNavigationReduxHelpers: 'mockReactNavigationReduxHelpers'}))
  ...
  describe('ManageConnection component', () => {
    let store = mockStore({
    ...
    it('should be handling functions', () => {
      ...
      let context = { store };
      let wrapper = shallow(
        <ManageConnection {...props} />,
        { context },
      );
      const inst = wrapper.dive().dive().dive().dive().instance();
      inst.apiCall()
      inst.onIndividualDelete({firstName: '', lastName: '', patientId: 2})
      inst.onGuardianDelete({coreoHomeUserId: 56, firstName: '', lastName: ''})
      inst.onClickButtonCancel()
      inst.onClickCancelIndividual()
      inst.onDelete()
      inst.onDeleteIndividual()
