...
class PerformTasks extends Component {
    ...
    completedTasksCount = () => {
        const {PerformTasksList} = this.props
        let completedTasks = 0;
        let totalTasksCount = 0;
        PerformTasksList && PerformTasksList.serviceRequestTypeVisits &&
        PerformTasksList.serviceRequestTypeVisits.map(service => {
            service.serviceRequestTypeTaskVisits.map(task => {
                if(task.statusId === COMPLETED_TASK_STATUS_ID || task.statusId === SERVICE_STATES.PAYMENT_PENDING) completedTasks++
                totalTasksCount++
            })
        })
    ...
    render() {
        ...
        return (
            <View>
                ...
                <PeformedTasks openedTasks={this.state.openedTasks} onPressTask={this.onPressTask} performTasksList={PerformTasksList} />
                </View>
