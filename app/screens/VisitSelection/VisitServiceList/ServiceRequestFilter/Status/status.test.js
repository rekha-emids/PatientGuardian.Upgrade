...
describe('Status component', () => {
    it('Status', () => {  
      ...
      const context = { store };
        wrapper = shallow(
          <Status customStatusData statusData={() => {}} types={[{serviceTypeId: 1, serviceTypeDescription: "hey"}]} {...props} onClickNext= {() => {}} data={[]} startOrStopService={() => {}} performTasksList={{visitStatus: 'IN_PROGRESS', visitStartTime: null, visitEndTime: null, visitTimeDuration: null}}/>,
          { context },
        ),
        inst = wrapper.instance()
        inst.setState({statusData:[{status: true}, {status: true}]})
        inst.onPress(0);
        inst.onPress(1);
      })
})
