...
describe('Cohorts component', () => {
    it('Cohorts', () => {  
      ...
      const context = { store };
        wrapper = shallow(
          <Cohorts types={[{serviceTypeId: 1, serviceTypeDescription: "hey"}]} {...props} onClickNext= {() => {}} data={[]} startOrStopService={() => {}} performTasksList={{visitStatus: 'IN_PROGRESS', visitStartTime: null, visitEndTime: null, visitTimeDuration: null}}/>,
          { context },
        ),
        inst = wrapper.dive().instance()
        inst.apiCall({pageNumber: 1, pageSize: 10})
      })
})